...
describe("train estimator", function () {
    ...
    beforeEach(() => {
        ...
        validTripDetails = new TripDetails('Bordeaux', 'Paris', thirtyOneDaysBeforeDate);
    ...
    describe('estimate', () => {
        ...
        it("should throw because start city is empty", async () => {
            ...
            await expect(async () => await service.estimate(tripRequest)).rejects.toThrowError(new InvalidTripInputException("Start city is invalid"));
        })

        it("should throw because destination city is empty", async () => {
            const tripDetails = new TripDetails('Bordeaux', '', thirtyOneDaysBeforeDate)
            const tripRequest = new TripRequest(tripDetails, [dummyPassenger]);

            await expect(async () => await service.estimate(tripRequest)).rejects.toThrowError(new InvalidTripInputException("Destination city is invalid"))
        })

        it("should throw because date is invalid", async () => {
            const tripDetails = new TripDetails('Bordeaux', 'Paris', new Date('2022/06/06'))
            const tripRequest = new TripRequest(tripDetails, [dummyPassenger]);

            await expect(async () => await service.estimate(tripRequest)).rejects.toThrowError(new InvalidTripInputException("Date is invalid"))
        })
        ...
        it('should throw ApiException when the API call fails', async () => {
            ...
            await expect(async () => await service.estimate(tripRequest)).rejects.toThrowError(ApiException);
        ...
        it("should throw because passager age is less than 0", async () => {
            ...
            await expect(async () => await service.estimate(tripRequest)).rejects.toThrowError(new InvalidTripInputException("Age is invalid"));
        })
        ...
        it("should increase by 30% if the ticket is bought five days before departure", async () => {
            const tripDetails = new TripDetails('Bordeaux', 'Paris', sixDaysBeforeDate);
            const tripRequest = new TripRequest(tripDetails, [dummyPassenger]);
        ...
        it("should not apply the couple discount because passengers doesn't have Couple discount", async () => {
            const tripRequest = new TripRequest(validTripDetails, [dummyPassenger, dummyPassenger]);
        ...
        it("should have 2 passengers a couple because one of passengers is strictly less than 18 yo", async () => {
            const passengerWithCoupleDiscount = new Passenger(17, [DiscountCard.Couple]);
            const tripRequest = new TripRequest(validTripDetails, [dummyPassenger, passengerWithCoupleDiscount]);
